...
my $server_xml_data;
my $script_name;
my $server_data_xml = "server-data.xml";
my $debug_output = "debug.txt";
...
sub show_users
{
	...
	$value = format_dollars($total_balance);
	print PIPE "\nTotal balance from listed users is $value\n";
	close PIPE;
...
MAIN:
{
	...
	$status = getopts("hdxp:",\%options);
	if ( $options{"h"} ) {
		display_pod_help($0);
		exit 0;
	} # IF
	unless ( $status && 2 < scalar @ARGV ) {
		die("Usage : $0 [-dhx] [-p pager] username password function [optional parameters]\n");
	} # UNLESS
	if ( $options{'x'} ) {
		unlink($server_data_xml);
	} # IF
	if ( $options{'d'} ) {
		unlink($debug_output);
	} # IF
...
smart.pl [-hdx] [-p pager] username password command parm_1 ... parm_n
...
=head1 PARAMETERS
  ...
  username - name of user issuing request
  password - password for user
  command - command requested by user
  parm_1 ... parm_n - optional parameters for command

=head1 OPTIONS

  -h - produce this summary
  -d - activate debugging mode
  -x - save XML data from server in the file server-data.xml
  -p pager - override default paging program
